...
// (REGLAS_MELOQUIZ.md §6): victoria por más aciertos y empate COMPARTIDO.
...
/** true = acierta, false = falla, null = no vota (se desconectó, §7). */
type PlanRonda = Readonly<Record<string, boolean | null>>;
...
function jugarRonda(estado: EstadoMeloquiz, plan: PlanRonda): EstadoMeloquiz {
  ...
  for (const [id, acierta] of Object.entries(plan)) {
    if (acierta === null) continue;
    const opcion = acierta ? opcionCorrecta(e) : opcionIncorrecta(e);
    e = exito(votar(e, id, opcion, reloj.avanzar(500)));
  }
...
describe("partida completa (§6)", () => {
  it("3 rondas: gana quien más acertó y el marcador cuadra", () => {
    let estado = partidaDe(3);
    estado = jugarRonda(estado, { j1: true, j2: true, j3: false });
    estado = jugarRonda(estado, { j1: true, j2: false, j3: false });
    estado = jugarRonda(estado, { j1: true, j2: false, j3: null });

    expect(estado.fase).toBe("final");
    expect(puntosDe(estado, "j1")).toBe(3);
  ...
  it("empate COMPARTIDO: los que empatan en el máximo comparten el puesto", () => {
    ...
    expect(puntosDe(estado, "j2")).toBe(2);
    expect(estado.ganadores).toEqual(["j1", "j2"]);
  });

  it("NADIE acierta nunca: todos quedan en 0 y todos empatan", () => {
    let estado = partidaDe(2);
    estado = jugarRonda(estado, { j1: false, j2: false, j3: false });
    estado = jugarRonda(estado, { j1: false, j2: false, j3: false });
  ...
  it("es reproducible: misma semilla y mismo plan ⇒ mismo resultado", () => {
    const jugar = (): EstadoMeloquiz => {
      ...
      e = jugarRonda(e, { j1: true, j2: false, j3: true });
      e = jugarRonda(e, { j1: false, j2: true, j3: true });
      return e;
    };
    const a = jugar();
    const b = jugar();
